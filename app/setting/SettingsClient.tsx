...
export default function SettingsClient() {
  ...
  // no ?tab= at all -> falls back to options[0] ("Profile Setting"), same as before
  const [selectedOption, setSelectedOption] = useState(() => {
    const tab = searchParams.get("tab");
    return (tab && TAB_PARAM_TO_LABEL[tab]) || options[0].label;
  });

  // if the query param changes while this page is already mounted, follow it
  useEffect(() => {
    const tab = searchParams.get("tab");
    if (tab && TAB_PARAM_TO_LABEL[tab]) {
      setSelectedOption(TAB_PARAM_TO_LABEL[tab]);
    }
  }, [searchParams]);
